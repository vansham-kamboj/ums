...
export const assignEventIncharge = async (req, res, next) => {
  try {
    const result = await service.assignEventIncharge(req.params, req.body, req.query, req.teamId, req.user);
    return successResponse(res, 'Operation successful', result);
  } catch (error) { next(error); }
};

export const getCombinedCalendar = async (req, res, next) => {
  try {
    const result = await service.getCombinedCalendar(req.params, req.body, req.query, req.teamId, req.user);
    return successResponse(res, 'Operation successful', result);
  } catch (error) { next(error); }
};
