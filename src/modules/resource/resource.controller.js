...
export const submitAssignment = async (req, res, next) => {
  try {
    const result = await service.submitAssignment(req.params, req.body, req.query, req.teamId, req.user);
    return successResponse(res, 'Operation successful', result);
  } catch (error) { next(error); }
};

export const evaluateAssignment = async (req, res, next) => {
  try {
    const result = await service.evaluateAssignment(req.params, req.body, req.query, req.teamId, req.user);
    return successResponse(res, 'Operation successful', result);
  } catch (error) { next(error); }
};

export const addSyllabusUnit = async (req, res, next) => {
  try {
    const result = await service.addSyllabusUnit(req.params, req.body, req.query, req.teamId, req.user);
    return successResponse(res, 'Operation successful', result);
  } catch (error) { next(error); }
};
