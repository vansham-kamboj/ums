...
export const addParticipant = async (req, res, next) => {
  try {
    const result = await service.addParticipant(req.params, req.body, req.query, req.teamId, req.user);
...
export const removeParticipant = async (req, res, next) => {
  try {
    const result = await service.removeParticipant(req.params, req.body, req.query, req.teamId, req.user);
    return successResponse(res, 'Operation successful', result);
  } catch (error) { next(error); }
};
