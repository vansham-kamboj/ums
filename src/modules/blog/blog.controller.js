...
export const listPublicBlogs = async (req, res, next) => {
  try {
    const result = await service.listPublicBlogs(req.params, req.body, req.query, req.teamId, req.user);
...
export const getPublicBlog = async (req, res, next) => {
  try {
    const result = await service.getPublicBlog(req.params, req.body, req.query, req.teamId, req.user);
    return successResponse(res, 'Operation successful', result);
  } catch (error) { next(error); }
};
