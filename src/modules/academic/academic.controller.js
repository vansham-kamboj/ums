...
export const archiveSession = async (req, res, next) => {
  try {
    const result = await service.archiveSession(req.params, req.body, req.query, req.teamId, req.user);
    return successResponse(res, 'Operation successful', result);
  } catch (error) { next(error); }
};

export const unarchiveSession = async (req, res, next) => {
  try {
    const result = await service.unarchiveSession(req.params, req.body, req.query, req.teamId, req.user);
    return successResponse(res, 'Operation successful', result);
  } catch (error) { next(error); }
};
...
export const importCourses = async (req, res, next) => {
  try {
    const result = await service.importCourses(req.params, req.body, req.query, req.teamId, req.user);
    return successResponse(res, 'Operation successful', result);
  } catch (error) { next(error); }
};

export const assignSubjectIncharge = async (req, res, next) => {
  try {
    const result = await service.assignSubjectIncharge(req.params, req.body, req.query, req.teamId, req.user);
    return successResponse(res, 'Operation successful', result);
  } catch (error) { next(error); }
};

export const mapStudentsToSubject = async (req, res, next) => {
  try {
    const result = await service.mapStudentsToSubject(req.params, req.body, req.query, req.teamId, req.user);
    return successResponse(res, 'Operation successful', result);
  } catch (error) { next(error); }
};
