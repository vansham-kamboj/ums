...
export const listPublicNews = async (req, res, next) => {
  try {
    const result = await service.listPublicNews(req.params, req.body, req.query, req.teamId, req.user);
    return successResponse(res, 'Operation successful', result);
  } catch (error) { next(error); }
};

export const getPublicNewsArticle = async (req, res, next) => {
  try {
    const result = await service.getPublicNewsArticle(req.params, req.body, req.query, req.teamId, req.user);
    return successResponse(res, 'Operation successful', result);
  } catch (error) { next(error); }
};
