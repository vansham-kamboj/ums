...
export const updateServiceRequestStatus = async (req, res, next) => {
  try {
    const result = await service.updateServiceRequestStatus(req.params, req.body, req.query, req.teamId, req.user);
    return successResponse(res, 'Operation successful', result);
  } catch (error) { next(error); }
};

export const reviewEditRequest = async (req, res, next) => {
  try {
    const result = await service.reviewEditRequest(req.params, req.body, req.query, req.teamId, req.user);
    return successResponse(res, 'Operation successful', result);
  } catch (error) { next(error); }
};
