import * as service from './reception.service.js';
...
export const assignComplaint = async (req, res, next) => {
  try {
    const result = await service.assignComplaint(req.params, req.body, req.query, req.teamId, req.user);
    return successResponse(res, 'Operation successful', result);
  } catch (error) { next(error); }
};

export const addComplaintLog = async (req, res, next) => {
  try {
    const result = await service.addComplaintLog(req.params, req.body, req.query, req.teamId, req.user);
    return successResponse(res, 'Operation successful', result);
  } catch (error) { next(error); }
};
...
export const resolveComplaint = async (req, res, next) => {
  try {
    const result = await service.resolveComplaint(req.params, req.body, req.query, req.teamId, req.user);
    return successResponse(res, 'Operation successful', result);
  } catch (error) { next(error); }
};
